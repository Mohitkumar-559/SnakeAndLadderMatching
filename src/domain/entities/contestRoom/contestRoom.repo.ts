...
class ContestRoomRepo {
    ...
    async getContestById(contestId: string) {
        ...
        let getContestList:any = await this.getContest('5');
        let contestData:ContestData;
        for(const contest of getContestList){
                if(contest.cid == contestId)
                {
                   contestData = contest 
                   break;
                }
        ...
        if (!contestData) {
            ...
            let newContestData:any;
            for(const contest of getContestList){
                if(contest.cid == contestId)
                {
                    newContestData = contest 
                }
            }
        
            newContestData.total_joined = contestCounters[parseInt(contestId)];
            const cacheKey = RedisKeys.ContestDetails("5");
            await this.redis.set(cacheKey, newContestData);
        }
        return contestData;
    ...
    async getContestCategorisationInCache(gameId: number) {
        ...
        const proc_name = "PROC_GET_SL_ContestCategorisation";
    ...
    async getContestPrizeBreakUp(contestId: number) {
        ...
        const proc_name = "PROC_GET_SL_ContestPrizeBreakup";
